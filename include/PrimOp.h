...
public:
  ...
  int getFreedom();
  // Return the freedom number on which the primary operator acts
...
#ifdef DEBUG_TRACE
    std::cout << "Instantiated IdentityOperator(freedom=0)." << std::endl;
#endif
  };
  IdentityOperator(int freedom) : PrimaryOperator(freedom,ALL) {
#ifdef DEBUG_TRACE
    std::cout << "Instantiated IdentityOperator(freedom="<<freedom<<")." << std::endl;
#endif
  };
...
#ifdef DEBUG_TRACE
    std::cout << "Instantiated NullOperator(freedom=0)." << std::endl;
#endif
  };
  NullOperator(int freedom) : PrimaryOperator(freedom,ALL) {
#ifdef DEBUG_TRACE
    std::cout << "Instantiated NullOperator(freedom="<<freedom<<")." << std::endl;
#endif
  };
  virtual void applyTo(State&,int,double);
};
